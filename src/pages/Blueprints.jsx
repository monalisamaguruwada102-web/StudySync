...
import { Layers, Rocket, Target, Clock, Zap, Star, LayoutGrid, ArrowRight, CheckCircle2, BadgeCheck, Plus } from 'lucide-react';
...
const Blueprints = () => {
    ...
    useEffect(() => {
        const fetchBlueprints = async () => {
            try {
                const res = await api.get('/api/premium/blueprints');
            ...
            <div className="max-w-6xl mx-auto space-y-12 py-6">
                ...
                {loading ? (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-64 bg-slate-100 dark:bg-slate-800 animate-pulse rounded-[3.5rem]" />
                        ))}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
